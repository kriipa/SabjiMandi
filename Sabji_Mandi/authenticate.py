...
from adminUser.models import AdminDb
from Mandi.models import Customer
...
class Authentication:
    ...
    def valid_customer_where_id(function):
        def wrap(request, customer_id):
            try:
                Customer.objects.get(
                    username=request.session['username'], password=request.session['password'])
                return function(request, customer_id)
            except:
                messages.error(
                    request, 'Please! Re-enter your email and password')
            return redirect('/login')
